...
def menu_ollama(ollama, models):
    ...
    if scelta.isdigit() and 1 <= int(scalta) <= len(models):
        model_name = models[int(scalta)-1]['name']
    else:
        # Cerca TinyLlama o primo disponibile
        model_name = next((m['name'] for m in models if 'tinyllama' in m['name'].lower()), models[0]['name'])
        ...
        try:
            ...
            if user_input.startswith(":finance"):
                ...
                if scelta == "1":
                    ...
                    t = int(input("  Anni: "))
                    result = finance.calculate('compound_interest', principal=p, rate=r, years=t)
                    print(f"  → {result.explanation}")
                elif scelta == "2":
                    ...
                    m = int(input("  Mesi: "))
                    result = finance.calculate('mortgage_payment', principal=p, monthly_rate=r, months=m)
                    print(f"  → {result.explanation}")
                continue
