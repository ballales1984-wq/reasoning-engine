...
def main():
    print("=" * 60)
    print("🧪 ReasoningEngine — Test Suite Completa")
    print("=" * 60)
    print()
    ...
    def test_power():
        ...
        return r['answer'] == 4.0
    total += 1
    if run_test("Potenza", test_power): passed += 1
    
    def test_sqrt():
        r = engine.math.solve('radice di 144')
        return r['answer'] == 12.0
    total += 1
    if run_test("Radice quadrata", test_sqrt): passed += 1
    
    def test_geometry():
        r = engine.math.solve('ipotenusa cateti 3 4')
        return abs(r['answer'] - 5.0) < 0.001
    total += 1
    if run_test("Pitagora", test_geometry): passed += 1
    ...
    total += 1
    if run_test("Deduttivo", test_deductive): passed += 1
    ...
    total += 1
    if run_test("Induttivo", test_inductive): passed += 1
    ...
    def test_analogical():
        ...
        return result.found
    total += 1
    if run_test("Analogico", test_analogical): passed += 1
    ...
    total += 1
    if run_test("Interesse semplice", test_simple_interest): passed += 1
    
    def test_compound_interest():
        r = finance.calculate("compound_interest", principal=1000, rate=0.05, years=2)
        return abs(r.result - 1102.50) < 0.01
    total += 1
    if run_test("Interesse composto", test_compound_interest): passed += 1
    
    def test_roi():
        r = finance.calculate("roi", gain=15000, cost=10000)
        return r.result == 50.0
    total += 1
    if run_test("ROI", test_roi): passed += 1
    
    def test_mortgage():
        r = finance.calculate("mortgage_payment", principal=200000, monthly_rate=0.003, months=360)
        return r.result > 0
    total += 1
    if run_test("Mutuo", test_mortgage): passed += 1
    
    def test_break_even():
        r = finance.calculate("break_even", fixed_costs=10000, price=50, variable_cost=30)
        return r.result == 500.0
    total += 1
    if run_test("Break-even", test_break_even): passed += 1
    
    def test_risk_reward():
        r = finance.calculate("risk_reward_ratio", potential_gain=300, potential_loss=100)
        return r.result == 3.0
    total += 1
    if run_test("Rischio/Rendimento", test_risk_reward): passed += 1
